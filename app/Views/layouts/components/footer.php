<footer class="footer">
    <div class="container-md">
        <div class="row">
            <div class="left-col col-lg-4">
                <section class="mb-3">
                    <a class="brand" href="<?= base_url('/'); ?>">
                        <img src="<?= base_url('/assets/brand-white.svg'); ?>" alt="brand" height="32">
                        <div class="text-brand">
                            <span class="primary">TOKUKAS</span>
                            <span class="secondary">Toko Buku Bekas</span>
                        </div>
                    </a>
                </section>
                <section class="mb-4">
                    <a href="https://www.google.com/maps/search/tokukas" target="_blank" class="d-flex gap-1">
                        <div class="icon-box">
                            <i class="material-icons">place</i>
                        </div>
                        <span>Jalan Pabean Kencana Raya No. 32, Desa Pabean Udik, Kec. Indramayu, Kab. Indramayu, Jawa Barat 45219, Indonesia.</span>
                    </a>
                </section>
                <section class="icon-bar mb-3">
                    <a href="mailto:tokukas@outlook.com" target="_blank">
                        <div class="icon-box">
                            <i class="material-icons">email</i>
                        </div>
                        <span>tokukas@outlook.com</span>
                    </a>
                    <div class="vr"></div>
                    <a href="https://t.me/tokukas" target="_blank">
                        <div class="icon-box">
                            <i class="fab fa-telegram fa-lg"></i>
                        </div>
                    </a>
                    <a href="https://signal.group/#CjQKIBOOTbfokZmxTuuAEuS-y-v0frkY6A3yjxdMJLh9f2OYEhDQWSDd7K8InBIEp7m1kYuY" target="_blank">
                        <div class="icon-box">
                            <img src="<?= base_url('/assets/signal-app-icon-white.svg'); ?>" alt="linktree icon" height="22">
                        </div>
                    </a>
                    <a href="https://linktr.ee/tokukas" target="_blank">
                        <div class="icon-box">
                            <img src="<?= base_url('/assets/linktree-icon-white.svg'); ?>" alt="linktree icon" height="22">
                        </div>
                    </a>
                </section>
            </div>
            <div class="right-col col-lg-4">
                <section class="mb-3">
                    <h2>Yang Bekas Pasti Lebih Murah!</h2>
                </section>
                <section>
                    <div class="footer-menus">
                        <a href="<?= base_url('/about'); ?>" class="menu">
                            <i class="material-icons">chevron_right</i>
                            <span>Tentang Kami</span>
                        </a>
                        <a href="<?= base_url('/bookoffer'); ?>" class="menu">
                            <i class="material-icons">chevron_right</i>
                            <span>Ajukan Penawaran</span>
                        </a>
                    </div>
                </section>
            </div>
            <div class="col-lg-4">
                <section class="mb-3 text-center">
                    <p><b>Kunjungi Toko Kami di :</b></p>
                </section>
                <section class="d-flex gap-3 justify-content-center">
                    <a href="https://tokopedia.com/tokukas" target="_blank"><img src="<?= base_url('/assets/tokopedia.png'); ?>" alt="" height="60"></a>
                    <a href="https://shopee.co.id/tokukas" target="_blank"><img src="<?= base_url('/assets/shopee.png'); ?>" alt="" height="60"></a>
                </section>
            </div>
        </div>
    </div>
    <div class="container-fluid bottom">
        <div class="container-md d-flex justify-content-between">
            <section>
                <span>&copy; 2021 - TOKUKAS</span>
            </section>
            <section class="d-inline-flex gap-2">
                <a href="<?= base_url('/terms'); ?>">Syarat Penggunaan</a>
                <div class="vr"></div>
                <a href="<?= base_url('/privacy'); ?>">Kebijakan Privasi</a>
            </section>
        </div>
    </div>
</footer>
